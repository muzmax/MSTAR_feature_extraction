...
from ..datasets.preprocessing import normalization,ToTensor
...
class mstar_knn():
    def __init__(self,norm:list,eval_path:str,labeled_path:str,pca:bool,pca_dim:int,device :str) -> None:
        assert len(norm) == 2
        transform = transforms.Compose([normalization(norm[0],norm[1]),ToTensor()])
        train_paths,eval_paths,_ = load_separated_paths(dir1=labeled_path,dir2=eval_path)
    ...
    def pca_ft(self, feat_train,feat_eval):
        ...
        print('Cumulative explained variation for {} principal components: {}'.format(self.pca_dim,np.sum(pca.explained_variance_ratio_)))

        feat_train_pca_np = pca_result[:feat_train.shape[0]]
        feat_eval_pca_np = pca_result[feat_train.shape[0]:]
        feat_train_pca = torch.from_numpy(feat_train_pca_np)
        feat_eval_pca = torch.from_numpy(feat_eval_pca_np)
        return feat_train_pca.to(self.device),feat_eval_pca.to(self.device)
        
    @torch.no_grad()
    def extract_features(self,model):

        # Exctract features from training set
        count = 1
        total_len = len(self.data_loader_train)
        features_train = None
        print('Exctracting labeled features ({} vectors) ...'.format(total_len))
        ...
        features_eval = None
        print('Exctracting evaluation features ({} vectors) ...'.format(total_len))
        ...
        if self.pca:
            features_train,features_eval = self.pca_ft(features_train,features_eval)
